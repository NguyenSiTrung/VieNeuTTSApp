...
verification approved 2026-09-21), Phase 5 Tasks 5.1, 5.2 and 5.3 complete, Phase 0 partial
(Task 0.3 needs release hardware). All commits are **local on `main`** — nothing has been pushed
(AGENTS.md Git Policy).
...
(device-less host; documented, not a regression). Latest full run: 1606 passed,
...
## Next: Phase 5 Task 5.4, then the Phase 5 manual checkpoint

Phase 4 is closed out (its manual-verification checkpoint `nqx.6.3` was approved 2026-09-21;
`plan.md` Phase 4 has no implementable task left). Phase 5's checkpoint (`nqx.7.5`) opens once
Task 5.4 lands. Continue with
**Phase 5 Task 5.4 — preserve Studio provenance and truthful re-synthesis**
(`nqx.7.4`: `src/vienetts_app/core/studio.py`, `src/vienetts_app/core/timeline.py`,
`src/vienetts_app/ui/controller.py`, `tests/unit/test_studio.py`,
`tests/unit/test_studio_controller.py`, `tests/unit/test_timeline.py`; persist the synthesis
context per clip while leaving editing/export engine-independent, require a matching active
profile for re-synthesis and expose a switch action instead of substituting another provider,
and test old projects, imported artifacts, save/load, mismatches and rollback). Task 5.3
(engine-safe audiobook/subtitle caches) is done in `5a49a2d`.

What Tasks 5.2/5.3 already give Task 5.4 (build on them, do not re-litigate):
...
- `bd` epic `VieNeuTTSApp-nqx`; Phase 3 tasks are `.5.x` (all closed, including the manual
  ...
  Phase 5 tasks are `.7.x` (`.7.1`, `.7.2` and `.7.3` closed; `.7.5` is the Phase 5
