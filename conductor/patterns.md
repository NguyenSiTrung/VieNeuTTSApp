...
Last refreshed: 2026-08-27 (elevations from phase03_corefeat_20260827)
...
- vieneu voice persistence: `save_voices(path)` writes ALL voices; there is NO custom-path LOAD api (init always reads site-packages assets). App-owned persistence = save to app-data JSON + merge-back into `tts._preset_voices` at engine init; `add_voice` must always pass `save=False` (from: phase03_corefeat_20260827, 2026-08-27).
