...
- **GGUF `Q8_0` / `Q4_K_M` (qwentts.cpp)** — a small native runtime pack
  ...
  native library cannot load (an interrupted install, a missing OS
  dependency), the engine reports it as an incomplete runtime and the
  Settings card offers **Repair** — a re-verified reinstall, not a stack
  trace.
- **Same voices, same clones.** The 9 CustomVoice speakers and Base clones
